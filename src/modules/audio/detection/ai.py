import numpy as np
import importlib.util
import sys
import librosa.feature

librosa.feature.melspectrogram

from src.modules.audio.streaming import GstChannel
from src.settings import SETTINGS


def load_module(file_path, module_name):
    spec = importlib.util.spec_from_file_location(module_name, file_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    return module


class ModelProxy:
    """
    Proxy class responsible for loading and interfacing with the AI model, allowing for dynamic loading of different models based on configuration.
    """

    def __init__(self, model_name: str = None):
        if not model_name:
            self._enable = False
            return

        self._enable = True
        self.model_name = model_name

        # Load the Python Module containg the Model declaration
        self._module = load_module(
            f"{SETTINGS.AI_MODELS_FOLDER}{self.model_name}/model.py", "external_module"
        )
        self._model = self._module.Model()

    def infer(self, audios: list[GstChannel]) -> tuple[np.ndarray, np.ndarray]:
        """
        Call the infer method of the loaded model to get predictions for the given audio samples.
        """
        if not self._enable:
            return np.zeros(len(audios), dtype=int), np.zeros(len(audios))

        predictions, probabilities = self._model.infer([audio[0] for audio in audios])
        return predictions, probabilities
